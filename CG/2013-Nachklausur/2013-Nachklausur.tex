...
\begin{figure}[h]
    ...
    \label{fig:1a}
\end{figure}
Anmerkung: da es sich um perfekte Spiegel handelt, sind die eingezeichneten Schattenstrahlen unnötig.
...
Die Anzahl der Schnitttests mit Primitiven kann durch Mailboxing weiter reduziert werden.                                  & \CheckedBox   & $\square$        & \CheckedBox       & \CheckedBox      \\
...
\begin{align}
    \lambda_1^\Delta &= \nicefrac{1}{6}  & \lambda_3^\Delta &= \nicefrac{1}{6}\\
    \lambda_4^\Delta &= \nicefrac{2}{3}
\end{align}

\[c' = \nicefrac{1}{6} \cdot c_1 + \nicefrac{1}{6} \cdot c_3 + \nicefrac{2}{3} \cdot c_4 \neq c\]

\section*{Aufgabe 8: OpenGL und OpenGL-Shader}
\begin{tabular}{cp{8cm}cc}\toprule
    ~  & Aussage                                                                                                                  & Wahr & Falsch \\\midrule
    1  & Beleuchtung mit Schattenberechnung kann bei OpenGL mit Hilfe von \texttt{glEnable(GL\_LIGHTING | GL\_SHADOWS)} aktiviert werden.  & $\square$      & \CheckedBox      \\
...
Bemerkung zu 1: falsch, da \texttt{GL\_SHADOWS} nicht existiert (\url{https://www.opengl.org/sdk/docs/man/html/glEnable.xhtml})
...
4 & Der Blendingoperator definiert durch \texttt{glBlendEquation(GL\_FUNC\_ADD)} und \texttt{glBlendFunc(GL\_SRC\_ALPHA, GL\_SRC\_ALPHA)} ist kommutativ.            &
 $\square$      & \CheckedBox   &  1) \\\bottomrule
\end{tabular}
\\[3pt]
1)
Da GL\_SRC\_ALPHA den Alpha-Wert des Quell-Fragments multipliziert und das 
beim Tauschen von Quell- und Ziel-Fragment nicht berücksichtigt wird, ist der
Operator nicht kommutativ.
